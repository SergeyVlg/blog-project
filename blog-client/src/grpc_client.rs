...
impl GrpcClient {
	...
	pub(super) async fn login(&self, name: String, password: String) -> Result<UserWithToken> {
	...
	pub(super) async fn create_post(&self, token: String, title: String, content: String) -> Result<Post> {
	...
	pub(super) async fn get_post(&self, post_id: Uuid) -> Result<Post> {
	...
	pub(super) async fn update_post(&self, token: String, post_id: Uuid, title: String, content: String) -> Result<Post> {
	...
	pub(super) async fn delete_post(&self, token: String, post_id: Uuid) -> Result<()> {
	...
	pub(super) async fn list_posts(&self, token: String, limit: u32, offset: u32) -> Result<GetPostsResponse> {
		let mut request = Request::new(ListPostsRequest { limit, offset });
		Self::add_bearer_token(&mut request, &token)?;
